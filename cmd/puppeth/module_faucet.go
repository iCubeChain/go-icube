...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/icube                                                            && \
	(cd $GOPATH/src/github.com/icube && git clone --depth=1 https://github.com/icube/go-icube) && \
  go build -v github.com/icube/go-icube/cmd/faucet                                              && \
