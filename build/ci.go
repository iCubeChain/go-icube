...
func doInstall(cmdline []string) {
	...
	if runtime.Version() < "go1.7" && !strings.Contains(runtime.Version(), "devel") {
		...
		log.Println("go-icube requires at least Go version 1.7 and cannot")
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Where to upload the source package (usually "ppa:icube/icube")`)
...
type debMetadata struct {
	...
	// go-icube version being built. Note that this
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time) debMetadata {
	if author == "" {
		...
		author = "Ethereum Builds <fjl@icube.org>"
	}
	return debMetadata{
		Env:         env,
		Author:      author,
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "--target", "android", "--javapkg", "org.icube", "-v", "github.com/icube/go-icube/mobile"))
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "--target", "ios", "--tags", "ios", "-v", "github.com/icube/go-icube/mobile")
