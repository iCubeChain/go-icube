...
import (
	...
	"github.com/icube/go-icube/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.icube", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-icube issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/icube/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.icube", "github.com/icube/go-icube/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "icube", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
