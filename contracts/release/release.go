...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	...
	if err := ctx.Service(&icube); err == nil {
		apiBackend = icube.ApiBackend
	} else {
		var icube *les.LightEthereum
		if err := ctx.Service(&icube); err == nil {
			apiBackend = icube.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/icube/go-icube/releases for new releases")
